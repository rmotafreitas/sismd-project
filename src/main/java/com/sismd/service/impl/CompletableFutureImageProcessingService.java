...
/**
 * Histogram equalization composed as an asynchronous CompletableFuture pipeline.
 *
...
public class CompletableFutureImageProcessingService implements ImageProcessingService {
    ...
    public ImageData process(ImageData input) {
        ...
        int totalPixels = w * h;
        int parts = Math.min(partitions, w);

        ExecutorService exec = Executors.newFixedThreadPool(parts);
        try {
            int[] hist       = computeHistogramAsync(input, w, h, parts, exec);
            int[] cumulative = HistogramUtils.buildCumulativeHistogram(hist);
            int   cdfMin     = HistogramUtils.firstNonZero(cumulative);
            Color[][] out    = applyEqualizationAsync(input, w, h, cumulative, totalPixels, cdfMin, parts, exec);
    ...
    @SuppressWarnings("unchecked")
    private Color[][] applyEqualizationAsync(ImageData img, int w, int h, int[] cumulative,
